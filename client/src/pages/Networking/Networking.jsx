    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 bg-white dark:bg-dark-card border border-gray-200 dark:border-gray-700 rounded-2xl px-5 py-3 shadow-sm">
            <FaUsers className="text-primary text-xl" />
            <div>
              <div className="text-xl font-bold text-text-primary dark:text-white">{connections.length}</div>
              <div className="text-xs font-medium text-text-secondary dark:text-gray-400">Connections</div>
            </div>
          </div>
        </div>
      </motion.div>
              ...
              {pendingRequests.map((req) => (
                <motion.div key={req._id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-text-primary dark:text-white">{req.sender?.name}</h3>
            ...
            {suggestions.map((user, index) => (
              ...
              >
                <Card className="flex flex-col items-center text-center p-6 border-2 border-transparent hover:border-primary/20 transition-all hover:shadow-lg group">
                  ...
                  <h3 className="text-base font-bold text-text-primary dark:text-white truncate w-full">{user.name}</h3>
                  ...
                  <button
                    onClick={() => dispatch(sendConnectionRequest(user._id))}
                    className="mt-4 w-full flex items-center justify-center gap-2 border-2 border-primary text-primary hover:bg-primary hover:text-white font-semibold text-sm py-2 px-4 rounded-xl transition-all duration-200"
                  >
              ...
              <motion.div key={user._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                <Card className="flex items-center gap-4 border-2 border-transparent hover:border-primary/20 transition-all hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-text-primary dark:text-white truncate">{user.name}</h3>
                    <p className="text-xs text-text-secondary dark:text-gray-400 truncate">{user.headline || 'SkillLinked Member'}</p>
                  </div>
                  <Button variant="outline" size="sm" className="text-xs shrink-0 rounded-xl">Message</Button>
                </Card>
              </motion.div>
            ))}
          </div>
